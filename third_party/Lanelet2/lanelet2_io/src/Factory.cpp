...
//===============================================================================================================
Parser::Ptr ParserFactory::create(
        const std::string& parserName, const Projector& projector, const io::Configuration& config)
{
    ParserFactory& inst = ParserFactory::instance();
    auto it = inst.registry_.find(parserName);

    std::cout<<parserName<<std::endl;
    ...
    throw UnsupportedIOHandlerError(
            "Requested parser " + parserName +
            " does not exist! Available parsers are: " + format(availableParsers()));
}

Parser::Ptr ParserFactory::createFromExtension(
        const std::string& extension, const Projector& projector, const io::Configuration& config)
{
    ParserFactory& inst = ParserFactory::instance();
...
//===============================================================================================================
Writer::Ptr WriterFactory::create(const std::string& writerName,
        const lanelet::Projector& projector, const io::Configuration& config)
{
    ...
    throw UnsupportedIOHandlerError(
            "Requested writer " + writerName +
            " does not exist! Available writer are: " + format(availableWriters()));
}

Writer::Ptr WriterFactory::createFromExtension(
        const std::string& extension, const Projector& projector, const io::Configuration& config)
{
    WriterFactory& inst = WriterFactory::instance();
    auto it = inst.extensionRegistry_.find(extension);
