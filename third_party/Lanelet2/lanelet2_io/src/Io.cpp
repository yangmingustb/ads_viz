...
std::unique_ptr<LaneletMap> load(const std::string& filename, const Origin& origin,
        ...
        ErrorMessages* errors, const io::Configuration& params)
{
    ...
    ErrorMessages err;
    io_handlers::Parser::Ptr parser =
            io_handlers::ParserFactory::createFromExtension(extension(filename), projector, params);
    auto map = parser->parse(filename, err);
...
std::unique_ptr<LaneletMap> load(const std::string& filename, const std::string& parserName,
        const Origin& origin, ErrorMessages* errors, const io::Configuration& params)
{
        ...
        const Projector& projector, ErrorMessages* errors, const io::Configuration& params)
{
    ...
    ErrorMessages err;
    io_handlers::Parser::Ptr p;

    p = io_handlers::ParserFactory::create(parserName, projector, params);
    auto map = p->parse(filename, err);
    handleErrorsOrThrow<ParseError>(err, errors);
    return map;
}
...
{
    return io_handlers::ParserFactory::availableParsers();
}

std::vector<std::string> supportedParserExtensions()
{
    return io_handlers::ParserFactory::availableExtensions();
}

void write(const std::string& filename, const LaneletMap& map, const Origin& origin,
        ErrorMessages* errors, const io::Configuration& params)
{
...
{
    ErrorMessages err;
    io_handlers::WriterFactory::createFromExtension(extension(filename), projector, params)
            ->write(filename, map, err);
...
{
    return io_handlers::WriterFactory::availableWriters();
}

std::vector<std::string> supportedWriterExtensions()
{
    return io_handlers::WriterFactory::availableExtensions();
}
}  // namespace lanelet
