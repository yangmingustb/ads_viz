...
{
    BasicPolygon2d ls(
            utils::concatenate(lss, [](const auto& elem) { return to2D(elem).basicLineString(); }));
...
template <typename PrimT> PrimT getDummy(Id id) { return PrimT(id); }
template <> RegulatoryElementPtr getDummy<RegulatoryElementPtr>(Id id)
{
...
public:
            ...
            const osm::File& file, const Projector& projector, ErrorMessages& errors)
    {
        ...
        return std::make_unique<LaneletMap>(loader.lanelets_, loader.areas_,
                loader.regulatoryElements_, loader.polygons_, loader.lineStrings_, loader.points_);
    }
...
private:
    ...
    void loadNodes(const lanelet::osm::Nodes& nodes, const Projector& projector)
    {
        for (const auto& nodeElem : nodes)
        {
            ...
            try
            {
                points_.emplace(node.id, Point3d(node.id, projector.forward(node.point),
    ...
    void loadRegulatoryElements(const osm::Relations& relations)
    {
        for (const auto& relElem : relations)
        {
            ...
            {
                parserError(id, "Creating a regulatory element of type "s + regelemType +
                                        " failed: " + e.what());
            }
        }
    }
    ...
    void addRegulatoryElements(std::vector<std::pair<PrimT, const osm::Relation*>>& addTos)
    {
        for (auto& addTo : addTos)
        {
            osm::forEachMember(addTo.second->members, RoleNameString::RegulatoryElement,
                    [&](const osm::Role& role) {
                        auto regElem = getOrGetDummy(
                                regulatoryElements_, role.second->id, addTo.first.id());
    ...
    // helper functions
    template <const char* Type> bool isType(const lanelet::osm::Relation& relation)
    {
        ...
        osm::forEachMember(roles, roleName, [&](auto& member) {
            if (member.second->type() != AttributeValueString::Way)
            {
                auto msg = roleName + " ring must consist of ways but id " +
                           std::to_string(member.second->id) + " is of type " +
                           member.second->type() + "!";
    ...
    RuleParameterMap getRulesForRegulatoryElement(Id currElemId, const osm::Roles& roles)
    {
        ...
        for (const auto& memberPair : roles)
        {
            ...
            else if (member->type() == AttributeValueString::Relation)
            {
                ...
                {
                    parserError(currElemId, "Relation refers to another relation "s +
                ...
                {
                    parserError(currElemId, "Regulatory element refers to another "
        ...
        while (!lineStrings.empty())
        {
            ...
            else
            {
                ...
                auto elem = std::find_if(
                        lineStrings.rbegin(), lineStrings.rend(), [lastId](const auto& elem) {
                            return elem.back().id() == lastId || elem.front().id() == lastId;
                        });
                // we are unable to close the current ring
                if (elem == lineStrings.rend())
                {
                    parserError(id, "Could not complete boundary around linestring " +
            ...
            const typename std::unordered_map<Id, PrimT>& map, Id id, Id currentPrimitiveId)
    {
        ...
        {
            parserError(
                    currentPrimitiveId, "Failed to get id "s + std::to_string(id) + " from map");
...
template <typename MapT> void registerIds(const MapT& map)
{
    if (!map.empty())
    {
        ...
        const std::string& filename, ErrorMessages& errors) const
{
    ...
    errors = buildErrorMessage("Errors ocurred while parsing Lanelet Map:",
            utils::concatenate({osmReadErrors, errors}));
    return map;
}

std::unique_ptr<LaneletMap> OsmParser::fromOsmFile(
        const osm::File& file, ErrorMessages& errors) const
{
