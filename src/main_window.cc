...
#define MEMBER_OFFSET(StructType, Member)                           \
    (size_t)(reinterpret_cast<char*>(                               \
                     &(reinterpret_cast<StructType*>(1)->Member)) - \
             1)
      ...
      _channelName2TypeMap()
{
    ...
    connect(ui_->actionAbout, SIGNAL(triggered(bool)), this,
            SLOT(showMessage()));
...
void MainWindow::calculateWH(void)
{
    ...
    if (count > 0)
    {
        ...
        for (VideoImgProxy* p : video_image_viewer_list_)
        {
            ...
            ui_->videoImageGridLayout->addWidget(
                    &p->video_image_viewer_, index / 3, index % 3, 1, 1);
            ++index;
        }
...
MainWindow::VideoImgProxy* MainWindow::AddVideoImgViewer()
{
    ...
    if (ret)
    {
        ...
        ret->video_image_viewer_.setSizePolicy(
...
void MainWindow::ActionAddGrid(void)
{
    ...
    {
        grid_shader_ = RenderableObject::CreateShaderProgram(
    ...
    if (grid_root_item_ == nullptr)
    {
        ...
        ui_->treeWidget->setItemWidget(
                grid_root_item_, 1, enable_grid_checkBox_);
...
void MainWindow::EnableRadarPoints(bool b)
{
    ...
    ui_->sceneWidget->setTempObjGroupEnabled(
            r->root_item_.text(0).toStdString(), b);
}
...
void MainWindow::ActionOpenRadarChannel(void)
{
    if (radar_points_shader_ == nullptr)
    {
        ...
        if (radar_points_shader_ != nullptr)
        {
            ui_->sceneWidget->AddNewShaderProg(
                    "radarpoints", radar_points_shader_);
        }
        else
        {
    ...
    if (closed_radarData_list_.empty())
    {
        ...
        ui_->treeWidget->setItemWidget(
                &radarProxy->root_item_, 1, &radarProxy->enable_checkBox_);

        ui_->treeWidget->setItemWidget(&radarProxy->channel_name_item_, 1,
                &radarProxy->channel_name_combobox_);
        ui_->treeWidget->setItemWidget(
                &radarProxy->action_item_, 1, &radarProxy->action_item_button_);
    ...
    connect(&radarProxy->channel_name_combobox_,
            SIGNAL(currentIndexChanged(int)), this, SLOT(ChangeRadarChannel()));
...
void MainWindow::DoOpenRadarChannel(bool b, RadarData* radarProxy)
{
    if (b)
    {
        ...
        //     auto radarcallback =
        //             [this, radarProxy](const std::shared_ptr<
        //                     apollo::drivers::RadarObstacles>& pdata) {
        ...
        //                 radarcallback,
        //                 radarProxy->channel_name_combobox_.currentText()
        //                         .toStdString(),
        //                 nodeName))
        //     {
        ...
        RadarData* radar)
{
    ...
    if (rawData != nullptr)
    {
        ...
        {
            std::cerr << "cannot create RadarPoints Renderable Object"
                      << std::endl;
        }
    }
}
...
void MainWindow::PlayRenderableObject(bool b)
{
    if (b)
    {
        ...
        //     auto pointCallback =
        //             [this](const std::shared_ptr<apollo::drivers::PointCloud>&
        //                             pdata) {
        ...
        pointcloud_top_item_->setToolTip(
                0, pointcloud_comboBox_->currentText());
        ...
        VideoImgProxy* theVideoImgProxy)
{
    ...
    {
        std::cerr << "----Dynamic Texture is nullptr or apollo.drivers.Image "
                     "is nullptr"
                  << std::endl;
    }
        ...
        VideoImgProxy* theVideoImgProxy)
{
    ...
    if (theVideoImgProxy->dynamic_texture_ != nullptr && imgData != nullptr)
    {
        ...
        {
            std::cerr << "-----------Cannot load compressed image from data "
                         "with QImage"
                      << std::endl;
        }
    }
...
void MainWindow::DoPlayVideoImage(bool b, VideoImgProxy* theVideoImg)
{
    if (b)
    {
        ...
        //     {
        //         theVideoImg->compressed_image_reader_ = new CyberChannReader<
        ...
        //     {
        //         auto videoCallback =
        //                 [this, theVideoImg](const std::shared_ptr<
        //                         apollo::drivers::CompressedImage>& pdata) {
        //                     this->ImageReaderCallback(pdata, theVideoImg);
        //                 };

        //         ret = theVideoImg->compressed_image_reader_
...
void MainWindow::ChangeRadarChannel(void)
{
    ...
    //     radar->channel_reader_->OpenChannel(
    //             obj->currentText().toStdString(), nodeName);
    // }
}
...
void MainWindow::AddNewWriter(const apollo::cyber::proto::RoleAttributes& role)
{
    ...
    {
        QMessageBox::warning(this, tr("Error"),
                tr("No Enough for New Channel!!!"), QMessageBox::Ok);
        return;
    }
