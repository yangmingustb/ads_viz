...
void SceneViewer::setTempObjGroupEnabled(
        const std::string &tmpObjGroupName, bool b)
{
...
bool SceneViewer::AddTempRenderableObj(
        const std::string &tmpObjGroupName, RenderableObject *renderObj)
{
    bool ret = false;
    ...
    if (!right_key_is_moved_ && event->button() == Qt::RightButton)
    {
        if (camera_dialog_ == nullptr)
        {
            ...
            else
            {
                ...
                connect(this, SIGNAL(CameraAttitudeChanged(QVector3D)),
                        camera_dialog_, SLOT(updateCameraAttitude(QVector3D)));
                connect(this, SIGNAL(CameraPosChanged(QVector3D)),
                        camera_dialog_, SLOT(updateCameraPos(QVector3D)));
            }
        }
