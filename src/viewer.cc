...
Viewer::Viewer():
        ...
        global_hints(new osg::TessellationHints),
        viewer(new osgViewer::Viewer),
        main_window_width(1000),
        main_window_height(600) {
    osg::DisplaySettings::instance()->setNumMultiSamples(4);
    global_hints->setDetailRatio(0.5);
    global_state_set->setMode(GL_LIGHTING, osg::StateAttribute::ON);
    osg::ref_ptr<osg::MatrixTransform> tf = new osg::MatrixTransform;
    // create the origin and axis of the global frame with length 5m for each axis
    createAxis(tf, 5.0, 0.05, 0.8);
    root->addChild(tf.get());
    viewer->addEventHandler(
            new osgGA::StateSetManipulator(
                    viewer->getCamera()->getOrCreateStateSet()));

    viewer->setUpViewInWindow(0, 0, main_window_width, main_window_height);
}


void Viewer::setWindowName(std::string window_name) {
    typedef osgViewer::Viewer::Windows Windows;
    Windows wds;
    viewer->getWindows(wds);
    for (Windows::iterator w = wds.begin(); w != wds.end(); ++w) {
        (*w)->setWindowName(window_name);
    }
}
osg::Geode* Viewer::createArrowGeode(double height,
                                     double radius,
...
void Viewer::setMaterialColor(osg::StateSet *state, const osg::Vec4 &color) {
    ...
    state->setAttribute(lm,
        osg::StateAttribute::ON | osg::StateAttribute::OVERRIDE);
}

void Viewer::createSphere(const osg::Vec3 &center, double radius, const osg::Vec4 &color)
{
    // create s shape
    osg::Geode *sg = new osg::Geode;
    osg::ShapeDrawable *s_shape = new osg::ShapeDrawable(new osg::Sphere(center, radius), global_hints);
...
void Viewer::createSphere(osg::Transform* tf,
        const osg::Vec3 &center, double radius, const osg::Vec4 &color)
{
    // create s shape
    osg::Geode *sg = new osg::Geode;
    tf->addChild(sg);
    osg::ShapeDrawable *s_shape = new osg::ShapeDrawable(
        new osg::Sphere(center, radius), global_hints);
...
void Viewer::createLineStrip(osg::Transform* tf,
...
void Viewer::createLineList(osg::Transform* tf,
                         osg::Vec3Array *vertices,
                         const osg::Vec4 &color,
                         double line_width)
{
...
void Viewer::createLine(osg::Transform* tf,
...
void Viewer::createText(osg::Transform* tf,
                        const osg::Vec3& pos,
                        double size,
                        std::string& label,
                        const osg::Vec4& color)
...
void Viewer::createLineLoop(osg::Transform* tf,
                            osg::Vec3Array *vertices,
                            const osg::Vec4 &color,
                            double line_width)
{
    ...
    // This time we simply use primitive, and hardwire the number of coords to use
    // since we know up front,
    linesGeom->addPrimitiveSet(new osg::DrawArrays(osg::PrimitiveSet::LINE_LOOP, 0, vertices->size()));
...
void Viewer::createRectangle(osg::Transform* tf,
    ...
    // This time we simply use primitive, and hardwire the number of coords to use
    // since we know up front,
    linesGeom->addPrimitiveSet(new osg::DrawArrays(osg::PrimitiveSet::LINE_LOOP, 0, osg_vertices->size()));
...
void Viewer::createFacesFromVertices(osg::Transform* tf,
                            osg::Vec3Array *vertices,
...
void Viewer::createMeshFromVertices(osg::Transform* tf,
                                    osg::Vec3Array *vertices,
                                    const osg::Vec4 &color,
                                    double line_width)
{
...
void Viewer::createMeshFromPath(osg::Transform* tf,
                            osg::Vec3Array *path,
                            const osg::Vec4 &color,
                            double line_width)
{
...
void Viewer::createWireFrameBox(osg::Transform* tf,
...
void Viewer::createPolygon(osg::Transform* tf,
                             osg::Vec3Array *vertices,
                             const osg::Vec4 &color,
                             double line_width) {
    osg::Geode* geode = new osg::Geode();
...
void Viewer::createWireFrameCube(osg::Transform* tf_root,
                                 osg::Transform* tf,
                                 const osg::Vec3 &center,
                                 const osg::Vec3 &sides,
                                 const osg::Vec4 &color) {
    root->addChild(tf_root);
    ...
    state->setAttributeAndModes(polyoffset, osg::StateAttribute::OVERRIDE|osg::StateAttribute::ON);
...
void Viewer::createWireFrameCube(osg::Transform* tf,
                                 const osg::Vec3 &center,
                                 const osg::Vec3 &sides,
                                 const osg::Vec4 &color)
{
...
void Viewer::createCube(osg::Transform* tf,
                    const osg::Vec3 &center,
                    const osg::Vec3 &sides,
                    const osg::Vec4 &color) {
    osg::Geode *geo = new osg::Geode;
...
void Viewer::createAxis(osg::Transform* previousJoint,
                        double length,
                        double radius,
                        double transparent)
{
...
void Viewer::createChildAxis(osg::Transform* tf,
                        double length,
                        double radius,
                        double transparent) {
...
void Viewer::setMainWindowSize(uint32_t width, uint32_t height) {
    main_window_height = height;
    main_window_width = width;
}

void Viewer::setDisplayData() {
